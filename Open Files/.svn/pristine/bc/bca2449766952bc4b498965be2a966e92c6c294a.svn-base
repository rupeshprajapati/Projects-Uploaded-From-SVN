IF EXISTS(SELECT [NAME] FROM SYSOBJECTS WHERE XTYPE='P' AND [NAME]='USP_REP_Pharma_SCH_H1_PUR')
BEGIN
	DROP PROCEDURE USP_REP_Pharma_SCH_H1_PUR
END
GO

CREATE PROCEDURE [USP_REP_Pharma_SCH_H1_PUR]
...
SET @SQLCOMMAND=RTRIM(@SQLCOMMAND)+' and IVW.DC_NO='' '' and ivw.entry_ty in (''PK'') and ps.schedule in (''H1'')'
