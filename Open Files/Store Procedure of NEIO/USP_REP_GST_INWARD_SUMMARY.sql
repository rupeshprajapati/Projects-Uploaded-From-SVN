IF EXISTS(SELECT NAME FROM SYSOBJECTS WHERE XTYPE='p' AND name = 'USP_REP_GST_INWARD_SUMMARY')
BEGIN
	DROP PROCEDURE USP_REP_GST_INWARD_SUMMARY
END
GO 
SET ANSI_NULLS ON
GO
...
-- =============================================
create PROCEDURE [dbo].[USP_REP_GST_INWARD_SUMMARY]
...
declare  Lother_cursor cursor for select (CASE WHEN bcode_nm <> '' THEN  bcode_nm ELSE Entry_ty END) as bcode_nm,Entry_ty   from LCODE  
--where Entry_ty in('PT','P1','PR','E1','GD','GC','C6','D6')   --Commented by Priyanka B on 24042019 for Bug-32424
where Entry_ty in('PT','P1','PR','E1','GD','GC','C6','D6','M3')  --Modified by Priyanka B on 24042019 for Bug-32424
open Lother_cursor
...
DEALLOCATE Lother_cursor
	...
	,c.code_nm as code_nm1 --Added by Priyanka B on 12032018 for Bug-31294
	, mon=month(b.date),yearr=year(b.date),monthh=datename(mm,b.date),b.date,b.inv_no,b.Entry_ty,b.Tran_cd
	,a.itserial  --Added by Priyanka B on 02042018 for Bug-31294
	...
	where 
	--a.entry_ty in('p1','pt') and --Commented by Priyanka B on 24042019 for Bug-32424
	(a.CGST_AMT+a.SGST_AMT+a.IGST_AMT) > 0  and b.date between @sdate and @edate
	...
	left outer join lcode c on (b.entry_ty = c.entry_ty)
	left outer join #tmpChrgtbl d on (a.Tran_cd =d.Tran_cd and a.entry_ty =d.entry_ty and a.itserial=d.itserial )
	where a.entry_ty ='E1' and (a.CGST_AMT+a.SGST_AMT+a.IGST_AMT) > 0  and b.date between @sdate and @edate
	group by a.cgst_per,a.sgst_per,a.igst_per,a.GSTRATE,b.date,b.inv_no,b.Entry_ty,b.Tran_cd,c.code_nm
	,a.itserial  --Added by Priyanka B on 02042018 for Bug-31294
	...
	left outer join lcode c on (b.entry_ty = c.entry_ty)
	left outer join #tmpChrgtbl d on (a.Tran_cd =d.Tran_cd and a.entry_ty =d.entry_ty and a.itserial=d.itserial )
	where  b.AGAINSTGS in('Service Purchase bill','Purchases') And b.entry_ty in('C6','GC')
	and (a.CGST_AMT+a.SGST_AMT+a.IGST_AMT) > 0  and b.date between @sdate and @edate
	group by a.cgst_per,a.sgst_per,a.igst_per,a.GSTRATE,b.date,b.inv_no,b.Entry_ty,b.Tran_cd,c.code_nm
	,a.itserial  --Added by Priyanka B on 02042018 for Bug-31294
	
	--Added by Priyanka B on 12032018 for Bug-31294 Start
	/*ISD INVOICE RECEIPT & */
	insert into #Gstsummary 
	...
	left outer join lcode c on (b.entry_ty = c.entry_ty)
	left outer join #tmpChrgtbl d on (a.Tran_cd =d.Tran_cd and a.entry_ty =d.entry_ty and a.itserial=d.itserial )
	where  B.entry_ty IN('J6') and (a.CGST_AMT+a.SGST_AMT+a.IGST_AMT) > 0  and b.date between @sdate and @edate
	group by a.cgst_per,a.sgst_per,a.igst_per,a.GSTRATE,b.date,b.inv_no,b.Entry_ty,b.Tran_cd,c.code_nm
	,a.itserial  --Added by Priyanka B on 02042018 for Bug-31294
	...
	left outer join lcode c on (b.entry_ty = c.entry_ty)
	left outer join #tmpChrgtbl d on (a.Tran_cd =d.Tran_cd and a.entry_ty =d.entry_ty and a.itserial=d.itserial )
	where  B.entry_ty IN('J8') and (a.CGST_AMT+a.SGST_AMT+a.IGST_AMT) > 0  and b.date between @sdate and @edate
	group by a.cgst_per,a.sgst_per,a.igst_per,a.GSTRATE,b.date,b.inv_no,b.Entry_ty,b.Tran_cd,c.code_nm
	,a.itserial  --Added by Priyanka B on 02042018 for Bug-31294
	
	--Commented by Priyanka B on 28052018 for Bug-31294 Start
	/*
	/*BANK PAYMENT ADVANCE*/

	insert into #Gstsummary 
	Select srno = 6 ,ORD = 'A', c.code_nm,c.code_nm as code_nm1, mon=month(b.date),yearr=year(b.date),monthh=datename(mm,b.date),b.date,b.inv_no,b.Entry_ty,b.Tran_cd
	,a.itserial  --Added by Priyanka B on 02042018 for Bug-31294
	,tot_deduc=isnull((sum(d.Fdisc_Befor_GST + d.Ded_Befor_GST)),0)
	,tot_tax=isnull(sum(d.add_Befor_GST + d.Tax_Befor_GST  +  d.NonTax_Befor_GST),0)
	,tot_add=0
	...
	left outer join lcode c on (b.entry_ty = c.entry_ty)
	left outer join #tmpChrgtbl d on (a.Tran_cd =d.Tran_cd and a.entry_ty =d.entry_ty and a.itserial=d.itserial )
	where  B.entry_ty IN('BP') and B.tdspaytype=2
	and (a.CGST_AMT+a.SGST_AMT+a.IGST_AMT) > 0  and b.date between @sdate and @edate
	group by a.cgst_per,a.sgst_per,a.igst_per,a.GSTRATE,b.date,b.inv_no,b.Entry_ty,b.Tran_cd,c.code_nm
	,a.itserial  --Added by Priyanka B on 02042018 for Bug-31294
	*/  --Commented by Priyanka B on 28052018 for Bug-31294 End
--Added by Priyanka B on 12032018 for Bug-31294 End

--Added by Priyanka B on 07042018 for Bug-31294 Start
	/*PURCHASE RETURN*/
	
	insert into #Gstsummary 
	Select srno = 7 ,ORD = 'B', c.code_nm
	,c.code_nm as code_nm1
	, mon=month(b.date),yearr=year(b.date),monthh=datename(mm,b.date),b.date,b.inv_no
	,b.Entry_ty,b.Tran_cd
	,a.itserial
	,tot_deduc=isnull((sum(d.Fdisc_Befor_GST + d.Ded_Befor_GST)),0)
	,tot_tax=isnull(sum(d.add_Befor_GST + d.Tax_Befor_GST  +  d.NonTax_Befor_GST),0)
	,tot_add=0
	...
	left outer join lcode c on (b.entry_ty = c.entry_ty)
	left outer join #tmpChrgtbl d on (a.Tran_cd =d.Tran_cd and a.entry_ty =d.entry_ty and a.itserial=d.itserial )
	where a.entry_ty ='pr' and (a.CGST_AMT+a.SGST_AMT+a.IGST_AMT) > 0  and b.date between @sdate and @edate
	group by a.cgst_per,a.sgst_per,a.igst_per,a.GSTRATE,b.date,b.inv_no,b.Entry_ty,b.Tran_cd,c.code_nm
	,a.itserial
--Added by Priyanka B on 07042018 for Bug-31294 End
	
	/*DEBIT NOTE FOR PURCHASES*/

	insert into #Gstsummary 
	Select srno = 8 ,ORD = 'B', c.code_nm
	,c.code_nm as code_nm1 --Added by Priyanka B on 12032018 for Bug-31294
	, mon=month(b.date),yearr=year(b.date),monthh=datename(mm,b.date),b.date,b.inv_no,b.Entry_ty,b.Tran_cd
	,a.itserial  --Added by Priyanka B on 02042018 for Bug-31294
	,tot_deduc=isnull((sum(d.Fdisc_Befor_GST + d.Ded_Befor_GST)),0)
	...
	left outer join lcode c on (b.entry_ty = c.entry_ty)
	left outer join #tmpChrgtbl d on (a.Tran_cd =d.Tran_cd and a.entry_ty =d.entry_ty and a.itserial=d.itserial )
	where  b.AGAINSTGS in('Service Purchase bill','Purchases') aND B.entry_ty IN('D6','GD')
	and (a.CGST_AMT+a.SGST_AMT+a.IGST_AMT) > 0  and b.date between @sdate and @edate
	group by a.cgst_per,a.sgst_per,a.igst_per,a.GSTRATE,b.date,b.inv_no,b.Entry_ty,b.Tran_cd,c.code_nm
	,a.itserial  --Added by Priyanka B on 02042018 for Bug-31294

--Added by Priyanka B on 02042018 for Bug-31294 Start
	Update a set 
...
--Added by Priyanka B on 02042018 for Bug-31294 End
