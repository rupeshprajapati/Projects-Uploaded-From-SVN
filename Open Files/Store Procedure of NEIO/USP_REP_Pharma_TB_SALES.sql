IF EXISTS(SELECT [NAME] FROM SYSOBJECTS WHERE XTYPE='P' AND [NAME]='USP_REP_Pharma_TB_SALES')
BEGIN
	DROP PROCEDURE USP_REP_Pharma_TB_SALES
END
GO

CREATE PROCEDURE [USP_REP_Pharma_TB_SALES]
...
SET @SQLCOMMAND='INSERT INTO #tbsales
...
SET @SQLCOMMAND=RTRIM(@SQLCOMMAND)+' and m.entry_ty=''SK'' and it_mast.prdisease=''TB'''
--and ps.schedule=''TB'''
PRINT @SQLCOMMAND
EXECUTE SP_EXECUTESQL @SQLCOMMAND

select srno,batchno,it_name,count(batchno) as totbatches,sum(qty) as totqty,state,city,others from #tbsales 
group by srno,it_name,batchno,state,city,others
order by it_name

--EXECUTE USP_REP_PHARMA_TB_SALES'','','','04/01/2018','03/31/2019','','','Crocin                                            ','Paracetamol                                       ',0,0,'','','','','','','','','2018-2019',''